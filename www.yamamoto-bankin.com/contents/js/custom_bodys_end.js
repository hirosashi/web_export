...
jQuery(document).ready(function($) {
	...
	/*スマホ時のTEL・MAPボタン2店舗の設定*/
	$('.sp-footer-tel').bind('click',function(){
		$('.tel-box').toggleClass('open');
		return false;
	});
  
	$(document).click(function(event) {
		if(!$(event.target).closest('.tel-box').length) {
			$('.tel-box').removeClass('open');
		}
	});
	$('.sp-footer-map').bind('click',function(){
		$('.map-box').toggleClass('open');
		return false;
  	});
	$(document).click(function(event) {
		if(!$(event.target).closest('.map-box').length) {
